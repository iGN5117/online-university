...
export function listLectures(classId: number): Lecture[] {
  const rows = getDb()
    .prepare(`${LECTURE_SELECT} WHERE l.class_id = ? ORDER BY l.position`)
    .all(classId) as LectureDbRow[];
  return rows.map(parseLecture);
}
...
export function getCatalogSummary(): string {
  ...
  for (const s of schools) {
    ...
    for (const c of classes) lines.push(`  class ${c.id}: ${c.name}`);
  }
  return lines.join("\n");
}
